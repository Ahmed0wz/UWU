...
// ============================================
// FILE-BASED STORAGE
// ── In-memory DB (source of truth while running)
// ── File System Access API writes back to user's chosen file
// ── FileSystemFileHandle persisted in IndexedDB so the app
//    reconnects silently on every reload — no prompt needed
// ============================================
...
const IDB_DB_NAME   = 'CalendarApp';
const IDB_STORE     = 'meta';
const IDB_KEY_HANDLE = 'fileHandle';
...
async function _flushSave() {
    ...
    if (_fileHandle) {
        try {
            ...
            if (perm !== 'granted') {
                ...
                if (req !== 'granted') { _fileHandle = null; await _idbClearHandle(); return; }
            }
    ...
    // No file handle — silent download fallback
...
function _silentDownload(json) {
    ...
    a.href = url; a.download = 'calendar-data.json'; a.click();
...
async function _idbClearHandle() {
    try {
        const db = await _idbOpen();
        const tx = db.transaction(IDB_STORE, 'readwrite');
        tx.objectStore(IDB_STORE).delete(IDB_KEY_HANDLE);
        db.close();
    } catch (e) {}
}

// ── Try to reconnect to the previously used file silently ────
// Returns true if connected, false if we need to show the load screen
async function _tryReconnectFile() {
    const handle = await _idbLoadHandle();
    if (!handle) return false;
    try {
        // Check permission — on mobile this may need a user gesture
        let perm = await handle.queryPermission({ mode: 'readwrite' });
        if (perm === 'prompt') {
            perm = await handle.requestPermission({ mode: 'readwrite' });
        }
        if (perm !== 'granted') return false;
        // Read the file and load into DB
        const file = await handle.getFile();
        const text = await file.text();
        _loadDBFromJSON(text);
        _fileHandle = handle;
        _showStorageBadge('✓ ' + handle.name + ' loaded');
        return true;
    } catch (e) {
...
// ── Public: open file picker and connect ─────────────────────
async function loadDataFile() {
    ...
    if (hasAPI) {
        try {
            ...
            if (perm !== 'granted') { alert('Write permission needed to auto-save.'); return false; }
            const file = await handle.getFile();
            _loadDBFromJSON(await file.text());
            _fileHandle = handle;
            await _idbSaveHandle(handle);
            _showStorageBadge('✓ ' + handle.name + ' — auto-saving');
            return true;
        } catch (e) {
        ...
        return new Promise(resolve => {
            ...
            input.onchange = async ev => {
                ...
                _loadDBFromJSON(await file.text());
                _showStorageBadge('✓ Loaded — downloads on save');
                resolve(true);
            };
            input.click();
        });
...
// ── Public: save/download immediately ────────────────────────
async function saveDataFileNow() {
    ...
    if (_fileHandle) {
        try {
            ...
            _showStorageBadge('✓ Saved'); return;
        } catch (e) { _fileHandle = null; await _idbClearHandle(); }
    }
    // Fallback: prompt Save As via File System Access API
    if (typeof window.showSaveFilePicker === 'function') {
        try {
            ...
            await _idbSaveHandle(_fileHandle);
            const writable = await _fileHandle.createWritable();
            await writable.write(json); await writable.close();
            _showStorageBadge('✓ Saved to ' + _fileHandle.name);
            return;
...
function _loadDBFromJSON(text) {
    ...
    } catch (e) { alert('Could not read data file — it may be corrupted.'); }
}

function _showStorageBadge(msg) {
    let b = document.getElementById('storageBadge');
    if (!b) {
        b = document.createElement('div');
        b.id = 'storageBadge';
        b.style.cssText = 'position:fixed;bottom:76px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.8);color:#fff;padding:5px 14px;border-radius:20px;font-size:12px;z-index:9999;pointer-events:none;transition:opacity 0.4s;white-space:nowrap';
...
window.addEventListener('beforeunload', () => {
    if (_savePending && !_fileHandle) {
        clearTimeout(_saveTimer);
        _silentDownload(JSON.stringify(DB, null, 2));
    }
});
...
document.addEventListener('DOMContentLoaded', async () => {
    // Try to silently reconnect to the last used file via IndexedDB
    const reconnected = await _tryReconnectFile();
    if (reconnected) {
        // File loaded silently — boot straight into app
        _finishInit();
    } else {
        // No remembered file — show the load/fresh screen
        _showDataLoadScreen();
    }
});
...
function _showDataLoadScreen() {
    ...
    overlay.innerHTML = `
        ...
        <button id="startFreshBtn"
            ...
            Start fresh (new file)
        </button>
        <div style="font-size:0.7rem;color:#555;margin-top:8px">
            ${hasFileAPI ? '✓ File remembered between sessions on this browser' : '⚠ This browser will prompt a download on each save'}
        </div>`;
