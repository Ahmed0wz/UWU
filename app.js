...
function updateModalHijriLabel(gregDateStr) {
    ...
    if (!label) return;
    if (!gregDateStr) { label.classList.add('hidden'); return; }
    if (state.calendarMode === 'hijri') {
        ...
        // In Gregorian mode show Hijri as secondary info
        const d = new Date(gregDateStr + 'T12:00:00');
        const h = toHijri(d);
        label.textContent = `${h.day} ${h.month} ${h.year} AH`;
        label.classList.remove('hidden');
...
function updateHeader(startOfWeek) {
    ...
    if (state.calendarMode === 'hijri') {
        ...
        const hm = islamicMonths.indexOf(h.month) + 1;
        if (currentMonth) currentMonth.textContent = islamicMonths[hm - 1];
        if (currentYear) currentYear.textContent = h.year + ' AH';
    } else {
...
// Hijri-aware month total: sum points for all Gregorian days that fall in the given Hijri month
function calcHijriMonthTotal(anyDateInMonth) {
    ...
    // Use the Gregorian month key of the first day for month tasks
...
function updateHeaderScores() {
    ...
    if (state.calendarMode === 'hijri') {
        const h = toHijri(state.currentDate);
        const hm = islamicMonths.indexOf(h.month) + 1;
        monthName = islamicMonths[hm - 1].substring(0, 3);
        year = h.year + ' AH';
    } else {
