...
function _showDataLoadScreen() {
    ...
    overlay.style.cssText = 'position:fixed;inset:0;z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;background:var(--bg-primary,#fff)';
    overlay.innerHTML = `
        <div style="font-size:2.5rem;">📅</div>
        <div style="font-size:1.3rem;font-weight:700;">Calendar</div>
        <div style="font-size:0.9rem;color:#666;text-align:center;max-width:300px">
            Your data is stored in a file on your device — nothing is sent to any server.
        </div>
        <button id="loadFileBtn" style="padding:12px 28px;background:#2563eb;color:#fff;border:none;border-radius:12px;font-size:1rem;font-weight:600;cursor:pointer;">
            📂 Load my data file
        </button>
        <button id="startFreshBtn" style="padding:8px 24px;background:transparent;border:1px solid #ccc;border-radius:10px;font-size:0.9rem;cursor:pointer;color:#666">
            Start fresh (new data)
        </button>
        <div style="font-size:0.75rem;color:#aaa;margin-top:4px">
            File System Access API: ${typeof window.showOpenFilePicker === 'function' ? '✓ Supported — file auto-saves' : '⚠ Not supported — you'll receive download prompts'}
        </div>`;
