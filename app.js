    ...
    try {
        ...
        if (e.name === 'QuotaExceededError' || e.name === 'NS_ERROR_DOM_QUOTA_REACHED') {
            if (!key.includes('background')) {
                alert('Storage quota exceeded. Consider removing the background image to free up space.');
            }
        } else {
...
function exportData() {
    ...
    const data = {
        ...
        background: { imageOmitted: hasBackground, opacity: state.backgroundOpacity, scope: state.backgroundScope },
    ...
    if (hasBackground) alert('Note: Background image was not included in the export to keep the file small.');
}
