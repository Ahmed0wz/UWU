...
function renderHijriMonthView() {
    ...
    if (currentYear) {
        ...
        currentYear.style.display = 'inline'; // always show Hijri year on mobile
    }

    // Inject sighting button next to (not inside) the month label — placed after the picker button
    ...
    // Icon-only on very small screens, with label on sm+
    sightingBtn.innerHTML = `<span>${offset !== 0 ? '🌙' : '🌑'}</span><span class="hidden sm:inline ml-1">${offset !== 0 ? 'Adjusted' : 'Sighting'}</span>`;
    // Insert it as a sibling AFTER the month-year picker button (not inside it)
    const yearEl = document.getElementById('currentYear');
    const pickerBtn = yearEl ? yearEl.closest('button[onclick*="openMonthYearPicker"]') : null;
    if (pickerBtn && pickerBtn.parentElement) {
        pickerBtn.parentElement.insertBefore(sightingBtn, pickerBtn.nextSibling);
    } else if (yearEl && yearEl.parentElement) {
        yearEl.parentElement.appendChild(sightingBtn);
    }
...
function updateHeader(startOfWeek) {
    ...
    if (state.calendarMode === 'hijri') {
        ...
        if (currentYear) {
            ...
            currentYear.style.display = 'inline'; // always show year in Hijri mode
        }
    } else {
        ...
        if (currentYear) {
            ...
            currentYear.style.display = ''; // restore responsive hiding
        }
    }
}
