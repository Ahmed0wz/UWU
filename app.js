...
function renderHijriMonthView() {
    ...
    if (currentYear) currentYear.textContent = hy + ' AH' + offsetLabel;

    // Inject sighting button into toolbar area (next to month label)
    ...
    sightingBtn.onclick = () => openHijriSightingPanel(hy, hm);
    sightingBtn.className = 'flex items-center space-x-1 px-2 py-1 rounded-lg border text-xs font-medium transition-colors ml-2 ' +
    ...
    sightingBtn.innerHTML = (offset !== 0 ? '🌙' : '🌑') + ' <span>' + (offset !== 0 ? 'Adjusted' : 'Sighting') + '</span>';
    // Append near the year display
    const yearEl = document.getElementById('currentYear');
    if (yearEl && yearEl.parentElement) yearEl.parentElement.appendChild(sightingBtn);
    ...
    for (let hd = 1; hd <= daysInMonth; hd++) {
        ...
        // Gregorian date as secondary label
        const gregLabel = `${gregDate.getDate()} ${['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][gregDate.getMonth()]}`;
...
function renderHijriYearView() {
    ...
    if (currentYear) currentYear.textContent = hy + ' AH';
    ...
    for (let hm = 1; hm <= 12; hm++) {
        ...
        const mPts = calcMonthTotal(dateToLocalString(hijriToGregorian(hy, hm, 1)));
...
function renderHijriMiniCalendar() {
    ...
    miniCalendarMonth.textContent = islamicMonths[hm - 1] + ' ' + hy;

    const firstDow = hijriMonthFirstDayOfWeek(hy, hm);
    const daysInMonth = hijriDaysInMonth(hy, hm);
    const todayStr = todayLocalString();

    let html = '<div class="grid grid-cols-7 gap-0.5 text-center text-xs font-medium theme-text-secondary mb-1">';
    ['S','M','T','W','T','F','S'].forEach(d => { html += `<div>${d}</div>`; });
    html += '</div><div class="grid grid-cols-7 gap-0.5 text-center text-xs">';
    for (let i = 0; i < firstDow; i++) html += '<div></div>';
    for (let hd = 1; hd <= daysInMonth; hd++) {
        const gregDate = hijriToGregorian(hy, hm, hd);
        const dateStr = dateToLocalString(gregDate);
        const isToday = dateStr === todayStr;
        const isSelected = dateStr === dateToLocalString(state.currentDate);
        html += `<button onclick="state.currentDate=new Date('${dateStr}T12:00:00');renderCurrentView();renderMiniCalendar();"
            class="aspect-square flex items-center justify-center rounded-full text-xs transition-colors
            ${isToday ? 'bg-blue-600 text-white font-bold' : ''}
            ${isSelected && !isToday ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 font-semibold' : ''}
...
function renderDayView() {
    ...
    if (state.calendarMode === 'hijri') {
        ...
        if (currentYear) currentYear.textContent = h.day + ' ' + islamicMonths[hm-1] + ' ' + h.year + ' AH (' + gregShort + ')';
    } else {
        if (currentMonth) currentMonth.textContent = dayNames[state.currentDate.getDay()];
...
function calcMonthTotal(dateStr) {
    ...
    for (let d = 1; d <= days; d++) {
        ...
        if (!seenWeeks.has(wk)) {
            seenWeeks.add(wk);
            // Credit week-task points when the week's Sunday falls in this month
