...
function renderSidebarContent() {
    ...
    sidebar.innerHTML = `
        ...
        <div class="px-4 py-3 border-b theme-border flex items-center justify-between shrink-0">
            <div>
                <h3 class="font-semibold">${date.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}</h3>
                <p class="text-xs theme-text-secondary">${hijri.day} ${hijri.month.substring(0, 3)} · <span class="font-semibold text-blue-600 dark:text-blue-400">${journal?.score || 0} pts</span></p>
            </div>
        ...
        <div class="flex border-b theme-border shrink-0">
            ...
            <button onclick="showSidebarEvents()" class="flex-1 py-2.5 text-sm font-medium transition-colors flex items-center justify-center gap-1.5
                ${state.sidebarView === 'events' ? 'border-b-2 border-blue-500 text-blue-600 dark:text-blue-400' : 'theme-text-secondary'}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                Events ${dayEvents.length > 0 ? `<span class="text-xs bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-300 rounded-full px-1.5">${dayEvents.length}</span>` : ''}
            </button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto min-h-0">
            ...
            <!-- Events tab -->
            <div class="${state.sidebarView === 'events' ? '' : (state.sidebarView === null ? '' : 'hidden')} p-4 animate-fade-in">
                ${state.sidebarView === null && !journal ? `
                    <div class="text-center py-8 theme-text-secondary">
                        <p class="text-sm mb-3">No journal entry yet</p>
                        <button onclick="showJournalEditor()" class="text-sm text-blue-600 dark:text-blue-400 hover:underline">Write one →</button>
                    </div>` : ''}
                ${state.sidebarView === null && journal ? `
                    <div class="space-y-3 mb-4">
                        ${(journal.title || journal.content) ? `
                        <div class="theme-bg-tertiary rounded-xl p-3">
                        ...
                        <div class="theme-bg-tertiary rounded-xl p-3 flex items-center justify-between">
                ...
                <h4 class="text-xs font-semibold theme-text-secondary uppercase tracking-wider mb-3">Events (${dayEvents.length})</h4>
                <div class="space-y-1">${renderSidebarEvents(dayEvents)}</div>
            </div>
        </div>

        <!-- Score footer — always visible when not in journal/score tab -->
        <div class="${state.sidebarView === 'journal' || state.sidebarView === 'score' ? 'hidden' : ''} shrink-0 px-4 py-3 border-t theme-border theme-bg-panel grid grid-cols-3 gap-2 text-center">
            <div>
                ...
                <div class="font-bold text-blue-600 dark:text-blue-400">${journal?.score || 0}</div>
            </div>
            <div>
...
function renderMonthTotal(dateStr) {
    ...
    return `<div class="flex items-center justify-between text-sm"><span class="theme-text-secondary">${monthName} Total</span><span class="font-medium">${total} pts</span></div>`;
}
...
function showSidebarEvents() {
    state.sidebarView = 'events';
    renderSidebarContent();
}
...
function searchGoToEvent(eventId) {
    const event = events.find(e => e.id === eventId);
    if (!event) return;
    closeSearchModal();
    ...
    renderMiniCalendar();
    switchView('day');
    state.currentDaySidebar = event.date;
