...
const state = {
    ...
    forgottenTasks: [],
    renderWindowDays: 31  // default: ~1 month forward+back
};
...
function loadAllData() {
    ...
    loadLocation(); loadPrayerTimesCache(); loadJournal(); loadVariables(); loadShowPrayerTimes(); loadTasks(); loadForgottenTasks(); loadRenderWindow();
}
...
function openRenderWindowSettings() {
    ...
    const options = document.querySelectorAll('.render-window-option');
    options.forEach(opt => {
        const val = parseInt(opt.dataset.days);
        const active = val === state.renderWindowDays;
        opt.classList.toggle('border-blue-500', active);
        opt.classList.toggle('bg-blue-50', active);
        opt.classList.toggle('dark:bg-blue-900/20', active);
    });
}
...
function setRenderWindow(days) {
    ...
    saveRenderWindow();
    // Update label in settings if panel is open
