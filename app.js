...
// Cascade rules:
//   day   → contributes to: day, week, month, year, all-time
//   week  → contributes to: week, month, year, all-time
//   month → contributes to: month, year, all-time
//   year  → contributes to: year, all-time
// ============================================
...
function loadTodos() {
    state.todos = loadFromStorage(STORAGE_KEYS.TODOS(state.currentUser)) || {};
}
...
function addTodo(scope, dateStr, text, points) {
    ...
    state.todos[key].push({ id: Date.now(), text, points: Number(points) || 0, completed: false });
    saveTodos();
}
...
// ── Points helpers ───────────────────────────────────────────
// Raw completed points for one exact scope key
function _scopePts(scope, dateStr) {
    return getTodos(scope, dateStr)
        .filter(t => t.completed && t.points)
        .reduce((s, t) => s + t.points, 0);
}

// Day scope only (used when aggregating up)
function _dayPts(dateStr) {
    return _scopePts('day', dateStr);
}

// Week scope only (used when aggregating up)
function _weekPts(dateStr) {
    return _scopePts('week', dateStr);
}

// Month scope only
function _monthPts(dateStr) {
    return _scopePts('month', dateStr);
}

// Year scope only
function _yearPts(year) {
    return (state.todos['year_' + year] || [])
        .filter(t => t.completed && t.points)
        .reduce((s, t) => s + t.points, 0);
}

// All days in a week → sum of day-scope tasks
function _allDayPtsInWeek(dateStr) {
    const d = new Date(dateStr + 'T12:00:00');
    const sun = new Date(d); sun.setDate(d.getDate() - d.getDay());
    let total = 0;
    for (let i = 0; i < 7; i++) {
        const day = new Date(sun); day.setDate(sun.getDate() + i);
        total += _dayPts(dateToLocalString(day));
    }
    return total;
}

// All days+weeks in a month → sum of day+week-scope tasks
function _allDayWeekPtsInMonth(dateStr) {
    const d = new Date(dateStr + 'T12:00:00');
    ...
    for (let i = 1; i <= days; i++) {
        ...
        total += _dayPts(ds);
        const wk = getTodoKey('week', ds);
        if (!seenWeeks.has(wk)) { seenWeeks.add(wk); total += _weekPts(ds); }
    }
    return total;
}

// All day+week+month tasks in a year
function _allDayWeekMonthPtsInYear(year) {
    let total = 0;
    const seenWeeks = new Set(), seenMonths = new Set();
    for (let m = 0; m < 12; m++) {
        const mStr = `${year}-${String(m+1).padStart(2,'0')}-01`;
        const mk = getTodoKey('month', mStr);
        if (!seenMonths.has(mk)) { seenMonths.add(mk); total += _monthPts(mStr); }
        const days = new Date(year, m + 1, 0).getDate();
        for (let i = 1; i <= days; i++) {
            const ds = `${year}-${String(m+1).padStart(2,'0')}-${String(i).padStart(2,'0')}`;
            total += _dayPts(ds);
            const wk = getTodoKey('week', ds);
            if (!seenWeeks.has(wk)) { seenWeeks.add(wk); total += _weekPts(ds); }
        }
...
// ── Public cascade getters ───────────────────────────────────

// For a single day display: day-scoped tasks only
function getTodoDayOnlyPoints(dateStr) {
    return _dayPts(dateStr);
}

// Week total: day tasks (all 7 days) + week tasks
function getTodoWeekPoints(dateStr) {
    return _allDayPtsInWeek(dateStr) + _weekPts(dateStr);
}

// Month total: day + week + month tasks
function getTodoMonthPoints(dateStr) {
    return _allDayWeekPtsInMonth(dateStr) + _monthPts(dateStr);
}

// Year total: day + week + month + year tasks
function getTodoYearPoints(year) {
    return _allDayWeekMonthPtsInYear(year) + _yearPts(year);
}

// All-time: every year that has any data
...
function showTodosEditor(dateStr) {
    ...
    if (!state.todoScope) state.todoScope = 'day';
...
function addTodoFromInput(scope, dateStr) {
    ...
    if (!text) return;
    addTodo(scope, dateStr, text, ptsEl?.value || 0);
    ...
    return `<div class="space-y-3">
        <div class="flex rounded-lg overflow-hidden border theme-border text-xs">
            ...
            <button onclick="state.todoScope='${s.id}'; renderSidebarContent();"
                class="flex-1 py-1.5 transition-colors ${active===s.id
        ...
        <div class="space-y-1 max-h-52 overflow-y-auto">
            ${todos.length === 0
                ? `<p class="text-sm theme-text-secondary text-center py-4">No ${active} tasks yet</p>`
                : todos.map(t => `
                <div class="flex items-center space-x-2 p-2 rounded-lg hover:theme-bg-tertiary group">
                    <input type="checkbox" ${t.completed ? 'checked' : ''}
                        ...
                        class="w-4 h-4 rounded border-gray-300 text-blue-500 flex-shrink-0 cursor-pointer">
                    ...
                    <button onclick="deleteTodo('${active}','${dateStr}',${t.id}); renderSidebarContent();"
                        class="opacity-0 group-hover:opacity-100 transition-opacity text-red-400 text-xs px-1 flex-shrink-0">✕</button>
                </div>`).join('')}
        </div>
        ${donePts > 0 ? `<div class="text-xs text-center theme-text-secondary">+${donePts} pts earned</div>` : ''}
        <div class="flex space-x-2 pt-1 border-t theme-border">
            <input type="text" id="newTodoText" placeholder="New task…"
                ...
                class="flex-1 theme-bg-tertiary border theme-border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 theme-text min-w-0">
            <input type="number" id="newTodoPts" placeholder="pts" min="0"
                class="w-14 theme-bg-tertiary border theme-border rounded-lg px-2 py-2 text-sm focus:ring-2 focus:ring-blue-500 theme-text flex-shrink-0">
            <button onclick="addTodoFromInput('${active}','${dateStr}');"
                class="px-3 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm font-medium transition-colors flex-shrink-0">+</button>
        </div>
    </div>`;
}
...
// Simple non-blocking info modal (replaces alert() in account flows)
function _showInfoModal(title, msg) {
    ...
    overlay.innerHTML = `
        <div class="theme-bg rounded-2xl shadow-2xl border theme-border w-full max-w-xs mx-4 p-5 space-y-4" style="animation:modalIn 0.18s ease-out">
            <h3 class="font-semibold">${title}</h3>
            <p class="text-sm theme-text-secondary">${msg}</p>
            <button class="w-full py-2 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium transition-colors">OK</button>
        </div>`;
...
function createNewUser() {
    // Build a proper modal instead of prompt()
    const overlay = document.createElement('div');
    ...
    overlay.innerHTML = `
        <div class="theme-bg rounded-2xl shadow-2xl border theme-border w-full max-w-xs mx-4 overflow-hidden" style="animation:modalIn 0.18s ease-out">
            <div class="px-5 py-4 border-b theme-border flex items-center justify-between">
                <h3 class="font-semibold text-base">Add Account</h3>
                <button id="addAccClose" class="p-1 hover:theme-bg-tertiary rounded-lg transition-colors theme-text-secondary">
            ...
            <div class="p-5 space-y-4">
                <div>
                    ...
                    <input id="addAccInput" type="text" placeholder="e.g. work, personal…"
                    ...
                    <p class="text-xs theme-text-secondary mt-1">Letters, numbers, hyphens and underscores only.</p>
                </div>
                <div id="addAccError" class="hidden text-xs text-red-500 -mt-1"></div>
                <div class="flex space-x-3 pt-1">
                    <button id="addAccCancel" class="flex-1 py-2 rounded-lg border theme-border text-sm font-medium hover:theme-bg-tertiary transition-colors">Cancel</button>
                    <button id="addAccConfirm" class="flex-1 py-2 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium transition-colors">Create</button>
                </div>
            </div>
        </div>`;
    document.body.appendChild(overlay);

    const input   = overlay.querySelector('#addAccInput');
    const errEl   = overlay.querySelector('#addAccError');
    const close   = () => overlay.remove();

    const confirm = () => {
        const username = input.value.trim();
        errEl.classList.add('hidden');
        if (!username) { errEl.textContent = 'Please enter a name.'; errEl.classList.remove('hidden'); return; }
        if (state.users.includes(username)) { errEl.textContent = 'Account already exists!'; errEl.classList.remove('hidden'); return; }
        if (!/^[a-zA-Z0-9_-]+$/.test(username)) { errEl.textContent = 'Use only letters, numbers, hyphens, and underscores.'; errEl.classList.remove('hidden'); return; }
    ...
    overlay.querySelector('#addAccClose').onclick   = close;
    overlay.querySelector('#addAccCancel').onclick  = close;
    overlay.querySelector('#addAccConfirm').onclick = confirm;
...
function deleteUser() {
    if (state.users.length <= 1) {
        _showInfoModal('Cannot Delete', 'You cannot delete the only account.'); return;
    }
    const overlay = document.createElement('div');
    overlay.id = 'delAccountOverlay';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px)';
    overlay.innerHTML = `
        <div class="theme-bg rounded-2xl shadow-2xl border theme-border w-full max-w-xs mx-4 overflow-hidden" style="animation:modalIn 0.18s ease-out">
            <div class="px-5 py-4 border-b theme-border flex items-center justify-between">
                <h3 class="font-semibold text-base">Delete Account</h3>
                <button id="delAccClose" class="p-1 hover:theme-bg-tertiary rounded-lg transition-colors theme-text-secondary">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <div class="p-5 space-y-4">
                <p class="text-sm theme-text-secondary">Select the account to permanently delete. All its data will be erased.</p>
                <div class="space-y-1" id="delAccList">
                    ${state.users.map(u => `
                    <button data-user="${u}" class="delAccBtn w-full flex items-center justify-between px-3 py-2.5 rounded-lg border-2 transition-colors text-sm
                        ${u === state.currentUser ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-transparent hover:theme-bg-tertiary theme-border'}">
                        <span class="font-medium">${u}</span>
                        ${u === state.currentUser ? '<span class="text-xs text-blue-500">current</span>' : ''}
                    </button>`).join('')}
                </div>
                <div id="delAccConfirmRow" class="hidden space-y-3 border-t theme-border pt-3">
                    <p class="text-sm text-red-500 font-medium">Delete "<span id="delAccName"></span>" and all its data?</p>
                    <div class="flex space-x-3">
                        <button id="delAccCancel" class="flex-1 py-2 rounded-lg border theme-border text-sm font-medium hover:theme-bg-tertiary transition-colors">Cancel</button>
                        <button id="delAccConfirm" class="flex-1 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm font-medium transition-colors">Delete</button>
                    </div>
                </div>
            </div>
        </div>`;
    ...
    overlay.querySelectorAll('.delAccBtn').forEach(btn => {
        btn.onclick = () => {
            selected = btn.dataset.user;
            overlay.querySelectorAll('.delAccBtn').forEach(b => { b.classList.remove('border-red-500','bg-red-50','dark:bg-red-900/20'); b.classList.add('border-transparent'); });
            btn.classList.remove('border-transparent'); btn.classList.add('border-red-500','bg-red-50','dark:bg-red-900/20');
            overlay.querySelector('#delAccName').textContent = selected;
            overlay.querySelector('#delAccConfirmRow').classList.remove('hidden');
        };
    });

    overlay.querySelector('#delAccClose').onclick   = close;
    overlay.querySelector('#delAccCancel').onclick  = close;
    overlay.querySelector('#delAccConfirm').onclick = () => {
        if (!selected) return;
        state.users = state.users.filter(u => u !== selected);
        saveUsers();
        [STORAGE_KEYS.EVENTS, STORAGE_KEYS.THEME, STORAGE_KEYS.BACKGROUND, STORAGE_KEYS.CALENDARS,
         STORAGE_KEYS.LOCATION, STORAGE_KEYS.PRAYER_TIMES_CACHE, STORAGE_KEYS.JOURNAL, STORAGE_KEYS.VARIABLES]
            .forEach(k => removeFromStorage(k(selected)));
        if (selected === state.currentUser) switchUser(state.users[0]);
        close();
    };
    overlay.addEventListener('click', e => { if (e.target === overlay) close(); });
}
...
function addNewCalendar() {
    const overlay = document.createElement('div');
    overlay.id = 'addCalOverlay';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px)';

    let chosenColor = CALENDAR_COLORS[Math.floor(Math.random() * CALENDAR_COLORS.length)].id;

    const colorDots = CALENDAR_COLORS.map(c =>
        `<button type="button" data-cid="${c.id}" class="addCalColor w-8 h-8 rounded-full ${c.bg} transition-transform hover:scale-110 ${c.id === chosenColor ? 'ring-2 ring-offset-2 ring-gray-400 dark:ring-gray-500' : ''}"></button>`
    ).join('');

    overlay.innerHTML = `
        <div class="theme-bg rounded-2xl shadow-2xl border theme-border w-full max-w-xs mx-4 overflow-hidden" style="animation:modalIn 0.18s ease-out">
            <div class="px-5 py-4 border-b theme-border flex items-center justify-between">
                <h3 class="font-semibold text-base">New Calendar</h3>
                <button id="addCalClose" class="p-1 hover:theme-bg-tertiary rounded-lg transition-colors theme-text-secondary">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <div class="p-5 space-y-4">
                <div>
                    <label class="block text-sm font-medium mb-1">Calendar name</label>
                    <input id="addCalInput" type="text" placeholder="e.g. Gym, Study…"
                        class="w-full theme-bg-tertiary border theme-border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 theme-text">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2">Color</label>
                    <div class="flex flex-wrap gap-2">${colorDots}</div>
                </div>
                <div id="addCalError" class="hidden text-xs text-red-500"></div>
                <div class="flex space-x-3 pt-1">
                    <button id="addCalCancel" class="flex-1 py-2 rounded-lg border theme-border text-sm font-medium hover:theme-bg-tertiary transition-colors">Cancel</button>
                    <button id="addCalConfirm" class="flex-1 py-2 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium transition-colors">Create</button>
                </div>
            </div>
        </div>`;
    document.body.appendChild(overlay);

    const input  = overlay.querySelector('#addCalInput');
    const errEl  = overlay.querySelector('#addCalError');
    const close  = () => overlay.remove();

    overlay.querySelectorAll('.addCalColor').forEach(btn => {
        btn.onclick = () => {
            chosenColor = btn.dataset.cid;
            overlay.querySelectorAll('.addCalColor').forEach(b => { b.classList.remove('ring-2','ring-offset-2','ring-gray-400','dark:ring-gray-500'); });
            btn.classList.add('ring-2','ring-offset-2','ring-gray-400','dark:ring-gray-500');
        };
    });
    ...
    const confirm = () => {
        ...
        state.calendars.push({ id, name, color: chosenColor });
        state.activeCalendars.push(id);
        saveCalendars();
        saveToStorage(STORAGE_KEYS.CALENDARS(state.currentUser) + '_active', state.activeCalendars);
        renderCalendarList(); renderCurrentView();
        close();
    };

    overlay.querySelector('#addCalClose').onclick   = close;
    overlay.querySelector('#addCalCancel').onclick  = close;
    overlay.querySelector('#addCalConfirm').onclick = confirm;
    input.addEventListener('keydown', e => { if (e.key === 'Enter') confirm(); if (e.key === 'Escape') close(); });
    overlay.addEventListener('click', e => { if (e.target === overlay) close(); });
    setTimeout(() => input.focus(), 60);
}
...
function renderWeekView() {
    ...
    renderAllDayRow(startOfWeek);
    renderWeekBody(startOfWeek);
    updateTimeIndicator();
}

function renderAllDayRow(startOfWeek) {
    document.getElementById('weekAllDayRow')?.remove();
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const dateStr = dateToLocalString(date);
        perDay.push({ dateStr, evts: events.filter(e => e.date === dateStr && e.isAllDay && state.activeCalendars.includes(e.calendar)) });
    }
    const hasAny = perDay.some(d => d.evts.length > 0);

    const row = document.createElement('div');
    row.id = 'weekAllDayRow';
    row.className = 'all-day-row shrink-0';

    const corner = document.createElement('div');
    corner.className = 'all-day-row-corner';
    if (hasAny) corner.innerHTML = '<span>all day</span>';
    row.appendChild(corner);

    perDay.forEach(({ dateStr, evts }) => {
        const cell = document.createElement('div');
        cell.className = 'all-day-cell';
        cell.addEventListener('click', (e) => {
            if (e.target === cell) openModal(null, dateStr, null);
        });
        evts.forEach(event => cell.appendChild(createEventElement(event)));
        row.appendChild(cell);
    });

    const weekView = document.getElementById('weekView');
    const weekBody = document.getElementById('weekBody');
    if (weekView && weekBody) weekView.insertBefore(row, weekBody);
}
...
function createDayColumn(dateStr, isToday, dayIndex) {
    ...
    // Only timed events here — all-day events are in the shared row above
    ...
    sidebar.innerHTML = `
        ...
        <div class="px-4 pt-3 pb-2 shrink-0">
            <div class="flex space-x-2">
                <button onclick="showJournalEditor()" class="flex-1 py-2 px-2 rounded-lg border theme-border hover:theme-bg-tertiary transition-colors flex items-center justify-center space-x-1 ${state.sidebarView === 'journal' ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500' : ''}">
                    <span>📝</span><span class="text-sm font-medium">Journal</span>
                </button>
                <button onclick="showScoreEditor()" class="flex-1 py-2 px-2 rounded-lg border theme-border hover:theme-bg-tertiary transition-colors flex items-center justify-center space-x-1 ${state.sidebarView === 'score' ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500' : ''}">
                    <span>⭐</span><span class="text-sm font-medium">Score</span>
                </button>
                <button onclick="showTodosEditor('${dateStr}')" class="flex-1 py-2 px-2 rounded-lg border theme-border hover:theme-bg-tertiary transition-colors flex items-center justify-center space-x-1 ${state.sidebarView === 'todos' ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500' : ''}">
                    <span>✅</span><span class="text-sm font-medium">Tasks</span>
                </button>
            </div>
        </div>
        <div class="flex-1 overflow-y-auto px-4 py-2 min-h-0">
            <div id="sidebarEditor" class="mb-3">${renderEditorContent(journal, autoScore)}</div>
            <div class="border-t theme-border pt-3">
                <h4 class="text-xs font-semibold theme-text-secondary uppercase tracking-wider mb-2">Events</h4>
        ...
        return `
            <div class="space-y-3 animate-fade-in">
                <input type="text" id="journalTitle" placeholder="Title (optional)" value="${journal?.title || ''}"
                    class="w-full theme-bg-tertiary border theme-border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 theme-text">
                <textarea id="journalContent" placeholder="Write about your day..." rows="4"
...
function calcAllTimeTotal() {
    const journalTotal = Object.values(state.journalEntries).reduce((sum, e) => sum + (e?.score || 0), 0);
    return journalTotal + getTodoAllTimePoints();
}
...
function applyBackground() {
    ...
    if (!state.backgroundImage) {
        bgLayer.style.backgroundImage = ''; bgLayer.style.opacity = '0';
...
function openSearch() {
    ...
    if (existing) { existing.querySelector('#searchInput')?.focus(); return; }

    const overlay = document.createElement('div');
    overlay.id = 'searchOverlay';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:9999;display:flex;align-items:flex-start;justify-content:center;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px);padding-top:max(60px,10vh)';
    overlay.innerHTML = `
        <div class="theme-bg rounded-2xl shadow-2xl border theme-border w-full max-w-md mx-4 overflow-hidden" style="animation:modalIn 0.18s ease-out">
            <div class="px-4 py-3 border-b theme-border flex items-center space-x-3">
                <svg class="w-4 h-4 theme-text-secondary flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0"/></svg>
                <input id="searchInput" type="text" placeholder="Search events…"
                    class="flex-1 bg-transparent border-none focus:ring-0 focus:outline-none text-sm theme-text placeholder-gray-400">
                <button id="searchClose" class="p-1 hover:theme-bg-tertiary rounded-lg transition-colors theme-text-secondary flex-shrink-0">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <div id="searchResults" class="max-h-80 overflow-y-auto">
                <div class="px-4 py-8 text-center text-sm theme-text-secondary">Type to search your events</div>
            </div>
        </div>`;
    document.body.appendChild(overlay);
    ...
    const doSearch = () => {
        ...
        if (!q) { results.innerHTML = '<div class="px-4 py-8 text-center text-sm theme-text-secondary">Type to search your events</div>'; return; }
        const found = events.filter(e =>
            e.title.toLowerCase().includes(q) ||
            (e.description && e.description.toLowerCase().includes(q))
        ).sort((a, b) => a.date.localeCompare(b.date));
        if (!found.length) {
            results.innerHTML = `<div class="px-4 py-8 text-center text-sm theme-text-secondary">No events found for "${input.value}"</div>`;
            return;
        }
        results.innerHTML = found.map(e => {
            const cal = state.calendars.find(c => c.id === e.calendar);
            const color = CALENDAR_COLORS.find(c => c.id === cal?.color);
            const dot = color ? `<span class="w-2.5 h-2.5 rounded-full flex-shrink-0 ${color.bg}"></span>` : '';
            const time = e.isAllDay ? 'All day' : e.startTime || '';
            return `<button onclick="openModal(events.find(ev=>ev.id==='${e.id}')); document.getElementById('searchOverlay')?.remove();"
                class="w-full flex items-center space-x-3 px-4 py-3 hover:theme-bg-tertiary transition-colors text-left border-b theme-border last:border-0">
                ${dot}
                <div class="flex-1 min-w-0">
                    <div class="text-sm font-medium truncate">${e.title}</div>
                    <div class="text-xs theme-text-secondary">${e.date}${time ? ' · ' + time : ''}</div>
                </div>
            </button>`;
        }).join('');
    };
    ...
    overlay.querySelector('#searchClose').onclick = close;
    overlay.addEventListener('click', e => { if (e.target === overlay) close(); });
    setTimeout(() => input.focus(), 60);
}
...
function clearAllData() {
    ...
    state.prayerTimesCache = {}; state.journalEntries = {}; state.variables = {}; state.todos = {};
