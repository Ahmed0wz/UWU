...
// ============================================================
// SECTIONS:
//  1.  Constants & Config      (STORAGE_KEYS, COLORS, THEMES)
//  2.  State                   (global state object + events)
//  3.  Date Utilities          (dateToLocalString, Islamic calendar)
//  4.  Prayer Times            (fetch, cache, refresh linked events)
//  5.  Storage                 (save/load helpers, journal, variables)
//  6.  Todo List               (CRUD, points, scoped keys)
//  7.  User & Data Management  (users, loadAll/saveAll, location)
//  8.  Theme & Appearance      (themes, dark mode, background)
//  9.  Event Modal             (open/close/save/delete events)
// 10.  Week View               (desktop 7-day + mobile 3-day)
// 11.  Month View
// 12.  Year View
// 13.  Mini Calendar
// 14.  Day View & Sidebar      (timeline, journal, score, todos)
// 15.  Score & Totals          (week/month/year/alltime calc)
// 16.  View Management         (switchView, navigate, animations)
// 17.  Background Settings
// 18.  Account & Settings Panel
// 19.  Search, Import/Export, Keyboard Shortcuts
// 20.  Utility Helpers
// 21.  Init & Event Listeners
// ============================================================
...
// ============================================
// PRAYER-BASED EVENT LIVE REFRESH
// When cached prayer times update, recalculate
// start/end times for all isPrayerBased events
// ============================================
async function refreshPrayerBasedEvents() {
    const prayerEvents = events.filter(e => e.isPrayerBased && e.prayerConfig);
    if (!prayerEvents.length || !state.userLocation) return;

    let changed = false;
    for (const ev of prayerEvents) {
        // Get prayer times for this event's date
        let timings = null;
        const d = new Date(ev.date + 'T12:00:00');
        if (state.userLocation.city && state.userLocation.country) {
            timings = await fetchPrayerTimesByCity(d, state.userLocation.city, state.userLocation.country);
        } else if (state.userLocation.lat && state.userLocation.lng) {
            timings = await fetchPrayerTimes(d, state.userLocation.lat, state.userLocation.lng);
        }
        if (!timings) continue;
        ...
        const newStart = minutesToTimeString(prayerMins + (startOffset || 0));
        const newEnd   = minutesToTimeString(prayerMins + (endOffset  || 30));

        if (ev.startTime !== newStart || ev.endTime !== newEnd) {
...
function savePrayerTimesCache() {
    ...
    // Refresh any events that derive their times from prayer times
    refreshPrayerBasedEvents();
}
...
// Get the key for a given scope + date
function getTodoKey(scope, dateStr) {
    ...
    if (scope === 'week') {
        ...
        return 'week_' + dateToLocalString(start);
    }
    if (scope === 'month') return 'month_' + dateStr.substring(0, 7);
...
function toggleTodo(scope, dateStr, id) {
    ...
    if (!list) return;
    // Use == (loose) so string "123" matches number 123 from JSON
...
// Sum of completed todo points for a date's day scope
function getTodoDayPoints(dateStr) {
    return (getTodos('day', dateStr))
        .filter(t => t.completed && t.points)
        .reduce((s, t) => s + t.points, 0);
}

// Scoped todo points used in week/month/year calc
function getTodoWeekPoints(dateStr) {
    ...
    // Week-scoped todos
    total += getTodos('week', dateStr).filter(t => t.completed && t.points).reduce((s,t) => s+t.points, 0);
    return total;
}
...
function getTodoMonthPoints(dateStr) {
    ...
    for (let i = 1; i <= days; i++) {
        ...
        total += getTodoDayPoints(key);
    }
    total += getTodos('month', dateStr).filter(t => t.completed && t.points).reduce((s,t) => s+t.points, 0);
    return total;
}
...
function getTodoYearPoints(year) {
    ...
    for (let m = 0; m < 12; m++) {
        ...
        total += getTodos('month', monthKey).filter(t => t.completed && t.points).reduce((s,t) => s+t.points, 0);
    }
    total += (state.todos[`year_${year}`] || []).filter(t => t.completed && t.points).reduce((s,t) => s+t.points, 0);
    return total;
}


// ============================================
// RENDER WINDOW — how many days around currentDate to expand repeats
// ============================================
function saveRenderWindow() {
    saveToStorage(STORAGE_KEYS.RENDER_WINDOW(state.currentUser), state.renderWindowDays);
}
function loadRenderWindow() {
    const v = loadFromStorage(STORAGE_KEYS.RENDER_WINDOW(state.currentUser));
    if (v && Number.isFinite(v)) state.renderWindowDays = v;
}

// Called whenever currentDate changes — removes stale repeat instances
// outside the new window, then re-expands base events within it.
function rebuildRenderWindow() {
    const anchor = state.currentDate;
    const halfDays = Math.ceil(state.renderWindowDays / 2);
    const winStart = new Date(anchor); winStart.setDate(anchor.getDate() - halfDays);
    const winEnd   = new Date(anchor); winEnd.setDate(anchor.getDate() + halfDays);

    // Drop all generated instances (keep base events: no parentId)
    events = events.filter(e => !e.parentId);

    // Re-expand each base repeating event within the window
    const bases = events.filter(e => e.repeat && e.repeat !== 'none');
    bases.forEach(base => generateRepeatEventsInWindow(base, winStart, winEnd));
...
function loadAllData() {
    ...
    loadLocation(); loadPrayerTimesCache(); loadJournal(); loadVariables(); loadShowPrayerTimes();
    loadTodos();
    loadRenderWindow();
}
...
function saveAllData() {
    ...
    saveLocation(); savePrayerTimesCache(); saveJournal(); saveVariables(); saveShowPrayerTimes();
    saveTodos();
    saveRenderWindow();
}
...
// ============================================
// GENERATE REPEAT EVENTS WITHIN A DATE WINDOW
// Only creates instances whose dates fall inside [winStart, winEnd].
// This replaces the old generateRepeatEvents for normal saves.
// ============================================
function generateRepeatEventsInWindow(baseEvent, winStart, winEnd) {
    const instances = [];
    const SAFETY_MAX = 1500; // generous upper bound
    ...
    while (occurrenceIndex < SAFETY_MAX) {
        ...
        switch (baseEvent.repeat) {
            ...
            case 'weekdays': {
                ...
                while (weekdaysAdded < occurrenceIndex) {
                    ...
                    if (searchDate.getDay() !== 0 && searchDate.getDay() !== 6) weekdaysAdded++;
                }
        ...
        // Skip anything before the window (but keep going — later dates may be in range)
        if (nextDate < winStart) continue;
        // Stop once we're past the window
        if (nextDate > winEnd) break;
...
// ============================================
// RESOLVE PRAYER-BASED EVENT TIME (sync, from cache)
// Returns { startTime, endTime } — uses cached prayer times so it's
// instant at render time. Falls back to stored times if not cached yet.
// ============================================
function resolveEventTimes(event, dateStr) {
    if (!event.isPrayerBased || !event.prayerConfig) {
        return { startTime: event.startTime, endTime: event.endTime };
    }
    const { prayer, startOffset, endOffset } = event.prayerConfig;
    // Try every possible cache key format for this date
    const timings = Object.entries(state.prayerTimesCache).find(([key]) =>
        key.startsWith(dateStr)
    );
    if (!timings) return { startTime: event.startTime, endTime: event.endTime };
    const prayerTimeStr = timings[1][prayer];
    if (!prayerTimeStr) return { startTime: event.startTime, endTime: event.endTime };
    const prayerMins = getPrayerTimeInMinutes(prayerTimeStr);
    return {
...
function createEventElement(event) {
    ...
    // Resolve times — for prayer-based events this reads from cache,
    // so repeated events always show the correct time for their specific date
    const { startTime: resolvedStart, endTime: resolvedEnd } = resolveEventTimes(event, event.date);
    const [startHour, startMin] = resolvedStart.split(':').map(Number);
    const [endHour, endMin] = resolvedEnd.split(':').map(Number);
    ...
    sidebar.innerHTML = `
        ...
        <div class="p-4 space-y-3 shrink-0">
            <div class="flex space-x-2">
                <button onclick="showJournalEditor()" class="flex-1 py-2 px-3 rounded-lg border theme-border hover:theme-bg-tertiary transition-colors flex items-center justify-center space-x-1.5 ${state.sidebarView === 'journal' ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500' : ''}">
                    <span>📝</span><span class="text-sm font-medium">Journal</span>
                </button>
                <button onclick="showScoreEditor()" class="flex-1 py-2 px-3 rounded-lg border theme-border hover:theme-bg-tertiary transition-colors flex items-center justify-center space-x-1.5 ${state.sidebarView === 'score' ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500' : ''}">
                    <span>⭐</span><span class="text-sm font-medium">Score</span>
                </button>
                <button onclick="showTodosEditor('${dateStr}')" class="flex-1 py-2 px-3 rounded-lg border theme-border hover:theme-bg-tertiary transition-colors flex items-center justify-center space-x-1.5 ${state.sidebarView === 'todos' ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500' : ''}">
...
function renderTodosEditor(dateStr) {
    ...
    const activeScope = state.todoScope || 'day';
    const todos = getTodos(activeScope, dateStr);
    const completedPts = todos.filter(t => t.completed && t.points).reduce((s,t) => s+t.points, 0);
    ...
    return `<div class="space-y-3 animate-fade-in">
        <div class="flex rounded-lg overflow-hidden border theme-border text-xs">
            ...
            <button onclick="state.todoScope='${s.id}'; renderSidebarContent();"
                class="flex-1 py-1.5 transition-colors ${activeScope === s.id ? 'bg-blue-500 text-white' : 'hover:theme-bg-tertiary theme-text-secondary'}">
                ${s.label}
            </button>`).join('')}
        </div>
        <div class="space-y-1.5 max-h-52 overflow-y-auto" id="todoList">
            ${todos.length === 0 ? '<p class="text-sm theme-text-secondary text-center py-4">No tasks yet</p>' :
              todos.map(t => `
              <div class="flex items-center space-x-2 p-2 rounded-lg hover:theme-bg-tertiary group">
                  <input type="checkbox" ${t.completed ? 'checked' : ''} onchange="toggleTodo('${activeScope}','${dateStr}',${t.id}); renderSidebarContent();"
                  ...
                  <button onclick="deleteTodo('${activeScope}','${dateStr}',${t.id}); renderSidebarContent();"
                      class="opacity-0 group-hover:opacity-100 transition-opacity text-red-500 hover:text-red-600 text-xs px-1">✕</button>
              </div>`).join('')}
        </div>
        ${completedPts > 0 ? `<div class="text-xs text-center theme-text-secondary">+${completedPts} pts from completed tasks</div>` : ''}
        <div class="flex space-x-2 pt-1 border-t theme-border">
            <input type="text" id="newTodoText" placeholder="New task..." onkeydown="if(event.key==='Enter')addTodoFromInput('${activeScope}','${dateStr}');"
            ...
            <button onclick="addTodoFromInput('${activeScope}','${dateStr}');"
...
function calcWeekJournalTotal(dateStr) {
    const date = new Date(dateStr + 'T12:00:00');
...
function renderWeekTotal(dateStr) {
    const weekTotal = calcWeekJournalTotal(dateStr) + getTodoWeekPoints(dateStr);
        ...
        <div class="p-2 space-y-1">
            ...
            <div class="px-3 py-2">
                <div class="flex items-center justify-between mb-1.5">
                    ...
                    <span class="text-xs theme-text-secondary" id="renderWindowLabel">${state.renderWindowDays === 1 ? '1 day' : state.renderWindowDays >= 365 ? '1 year' : state.renderWindowDays + ' days'}</span>
                </div>
                <input type="range" min="0" max="5" step="1"
                    value="${['7','14','30','90','180','365'].indexOf(String(state.renderWindowDays)) >= 0 ? ['7','14','30','90','180','365'].indexOf(String(state.renderWindowDays)) : 2}"
                    oninput="setRenderWindow(this.value)"
                    class="opacity-slider w-full">
                <div class="flex justify-between text-[10px] theme-text-secondary mt-1">
...
function setRenderWindow(sliderVal) {
    const options = [7, 14, 30, 90, 180, 365];
    state.renderWindowDays = options[parseInt(sliderVal)] || 30;
    saveRenderWindow();
    const label = document.getElementById('renderWindowLabel');
    if (label) {
        const d = state.renderWindowDays;
        label.textContent = d === 7 ? '1 week' : d === 14 ? '2 weeks' : d === 30 ? '1 month' : d === 90 ? '3 months' : d === 180 ? '6 months' : '1 year';
    }
    rebuildRenderWindow();
    renderCurrentView();
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    loadAllData();
    await initLocation();
    rebuildRenderWindow(); // build initial window around today
    renderWeekView();
    renderMiniCalendar();
    updateTimeIndicator();
