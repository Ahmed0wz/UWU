...
function loadAllData() {
    ...
    loadLocation(); loadPrayerTimesCache(); loadJournal(); loadVariables(); loadShowPrayerTimes(); loadTasks(); loadForgottenTasks();
}
...
function renderWeekAllDayRow(startOfWeek, numDays) {
    ...
    for (let i = 0; i < numDays; i++) {
        ...
        date.setDate(startOfWeek.getDate() + i);
        const dateStr = dateToLocalString(date);
        const allDayEvts = events.filter(e => e.date === dateStr && e.isAllDay && state.activeCalendars.includes(e.calendar));
...
function handleKeyboardShortcuts(e) {
    if (e.key === 'Escape') {
        ...
        closeUserMenu(); closeSettings(); closeLocationSettings(); closeForgottenTasks(); closeEditSeriesModal();
