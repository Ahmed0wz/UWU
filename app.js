...
function savePrayerTimesCache() { saveToStorage(STORAGE_KEYS.PRAYER_TIMES_CACHE(state.currentUser), state.prayerTimesCache); }
...
function toggleTheme() { applyTheme(state.currentTheme || 'default', state.isDarkMode ? 'light' : 'dark'); saveTheme(); }
...
function renderSidebarMiniCalendar(selectedDateStr) {
    const sel = new Date(selectedDateStr + 'T12:00:00');
    const todayStr = todayLocalString();

    if (state.calendarMode === 'hijri') {
        const h = toHijri(sel);
        const hy = h.yearNum, hm = h.monthNum;
        const days = hijriDaysInMonth(hy, hm);
        const firstDow = hijriMonthFirstDayOfWeek(hy, hm);
        const monthLabel = h.month + ' ' + hy + ' AH';
        let cells = '';
        for (let i = 0; i < firstDow; i++) cells += '<div></div>';
        for (let d = 1; d <= days; d++) {
            const gregDate = hijriToGregorian(hy, hm, d);
            const dStr = dateToLocalString(gregDate);
            const isToday = dStr === todayStr;
            const isSel = dStr === selectedDateStr;
            cells += `<button onclick="sidebarNavToDate('${dStr}')"
                class="aspect-square w-full rounded-full text-[10px] font-medium flex items-center justify-center transition-colors
                ${isSel ? 'bg-blue-500 text-white' : isToday ? 'ring-1 ring-blue-400 text-blue-600 dark:text-blue-400 font-bold' : 'hover:theme-bg-tertiary theme-text'}">${d}</button>`;
        }
        return `
        <div class="flex items-center justify-between mb-1.5">
            <button onclick="sidebarNavMonth(-1)" class="p-1 hover:theme-bg-tertiary rounded"><svg class="w-3 h-3 theme-text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg></button>
            <span class="text-xs font-semibold theme-text">${monthLabel}</span>
            <button onclick="sidebarNavMonth(1)" class="p-1 hover:theme-bg-tertiary rounded"><svg class="w-3 h-3 theme-text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg></button>
        </div>
        <div class="grid grid-cols-7 gap-0.5 text-center">
            ${['S','M','T','W','T','F','S'].map(d => `<div class="text-[9px] font-semibold theme-text-secondary py-0.5">${d}</div>`).join('')}
            ${cells}
        </div>`;
    }
    ...
    const firstDay = new Date(year, month, 1).getDay();
    const daysInMonth = new Date(year, month + 1, 0).getDate();
    const monthLabel = sel.toLocaleDateString('en-US', { month: 'short', year: 'numeric' });
    let cells = '';
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        const isToday = dStr === todayStr;
        const isSel = dStr === selectedDateStr;
        const hasScore = !!state.journalEntries[dStr]?.score;
        cells += `<button onclick="sidebarNavToDate('${dStr}')"
            class="aspect-square w-full rounded-full text-[10px] font-medium flex items-center justify-center transition-colors relative
            ${isSel ? 'bg-blue-500 text-white' : isToday ? 'ring-1 ring-blue-400 text-blue-600 dark:text-blue-400 font-bold' : 'hover:theme-bg-tertiary theme-text'}">
            ${d}${hasScore && !isSel ? '<span class="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-blue-400"></span>' : ''}
        </button>`;
    }
    return `
    <div class="flex items-center justify-between mb-1.5">
...
function sidebarNavToDate(dateStr) {
    state.currentDate = new Date(dateStr + 'T12:00:00');
    state.currentDaySidebar = dateStr;
    renderMiniCalendar();
    renderSidebarContent();
    if (state.currentView === 'day') renderCurrentView();
}

function sidebarNavMonth(delta) {
    const cur = new Date(state.currentDaySidebar + 'T12:00:00');
    if (state.calendarMode === 'hijri') {
        const h = toHijri(cur);
        let hm = h.monthNum + delta, hy = h.yearNum;
        if (hm > 12) { hm = 1; hy++; }
        if (hm < 1)  { hm = 12; hy--; }
        const greg = hijriToGregorian(hy, hm, 1);
        state.currentDaySidebar = dateToLocalString(greg);
    } else {
        cur.setDate(1);
        cur.setMonth(cur.getMonth() + delta);
        state.currentDaySidebar = dateToLocalString(cur);
    }
    renderSidebarContent();
}
...
function renderWeekTotal(dateStr) {
    return `<div class="flex items-center justify-between text-sm"><span class="theme-text-secondary">Week Total</span><span class="font-medium">${getWeekTotal(dateStr)} pts</span></div>`;
}
...
function renderAllTimeTotal() {
    return `<div class="flex items-center justify-between text-sm"><span class="theme-text-secondary font-medium">All-Time Total</span><span class="font-bold text-blue-600 dark:text-blue-400">${calcAllTimeTotal()} pts</span></div>`;
}
...
function openForgottenTasks() {
    ...
    $('forgottenTasksModal').classList.remove('hidden');
}

function closeForgottenTasks() { $('forgottenTasksModal')?.classList.add('hidden'); }
...
function openViewAllTasks() {
    ...
    const sections = [];
    const scopeLabels = { day: 'Daily', week: 'Weekly', month: 'Monthly', year: 'Yearly' };
    for (const scope of ['day', 'week', 'month', 'year']) {
        const entries = [];
    ...
    const bodyHTML = total === 0
        ? `<div class="text-center py-12 theme-text-secondary">
            ...
            <p class="text-sm mt-1 opacity-70">No uncompleted tasks across any period</p>
           </div>`
        : sections.map(sec => `
            <div class="px-5 pt-4 pb-1">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex items-center gap-3 py-2 px-3 theme-bg-tertiary rounded-xl">
                        <div class="flex-1 min-w-0">
                            <div class="text-sm font-medium theme-text truncate">${task.name}</div>
                            <div class="text-xs theme-text-secondary">${key}${task.points ? ' · ' + task.points + ' pts' : ''}</div>
                        </div>
    ...
    modal.innerHTML = `
        <div class="theme-bg rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] flex flex-col border theme-border overflow-hidden">
            <div class="px-5 py-4 border-b theme-border flex items-center justify-between shrink-0">
                <div>
                    <h2 class="font-bold text-base">All Pending Tasks</h2>
                    <p class="text-xs theme-text-secondary mt-0.5">${total} uncompleted task${total !== 1 ? 's' : ''} across all periods</p>
                </div>
...
function createSettingsMenu() {
    ...
    menu.innerHTML = `
        <div class="p-2 space-y-1">
            ...
            <button onclick="exportICS()" class="w-full flex items-center space-x-3 px-3 py-2 rounded-lg hover:theme-bg-tertiary transition-colors text-left text-sm">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                <span>Export to Calendar (.ics)</span>
            </button>
...
function openSearch() {
    ...
    if (!modal) return;
    showEl('searchModal');
    // Inject tab bar once (between header and results)
    if (!$('searchTabBar')) {
    ...
    if (results) results.innerHTML = '<div class="px-5 py-8 text-center theme-text-secondary text-sm">Start typing to search</div>';
...
function setSearchTab(tab) {
    ...
    if (q.trim()) performSearch(q);
        ...
        if (r) r.innerHTML = '<div class="px-5 py-8 text-center theme-text-secondary text-sm">Start typing to search</div>';
    }
}
...
function performSearch(query) {
    ...
    if (!resultsEl) return;
    if (!query.trim()) {
        resultsEl.innerHTML = '<div class="px-5 py-8 text-center theme-text-secondary text-sm">Start typing to search</div>';
        return;
    }
    ...
    if (state._searchTab === 'journal') {
        ...
        if (!matches.length) {
            resultsEl.innerHTML = `<div class="px-5 py-8 text-center theme-text-secondary text-sm">No journal entries found for "<strong>${query}</strong>"</div>`;
            return;
        }
            ...
            <button onclick="searchGoToJournal('${dateStr}')" class="w-full px-5 py-3 flex items-start space-x-3 hover:theme-bg-tertiary transition-colors text-left">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                        ...
                        ${j.score ? `<span class="text-xs font-bold text-blue-500">${j.score}pts</span>` : ''}
                    </div>
                    <div class="text-xs theme-text-secondary mb-1">${dateStr} · ${hijri.day} ${hijri.month.slice(0,3)} ${hijri.year}</div>
