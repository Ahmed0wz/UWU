...
const state = {
    ...
    sidebarView: null,
    recurringRenderDays: 30   // default render window: 1 month ahead from current view
};
...
// ============================================
// RENDER WINDOW & LAZY RECURRING EXPANSION
// ============================================

function loadRenderWindow() {
    const saved = loadFromStorage(STORAGE_KEYS.RENDER_WINDOW(state.currentUser));
    if (saved !== null && saved !== undefined) state.recurringRenderDays = saved;
}
function saveRenderWindow() {
    saveToStorage(STORAGE_KEYS.RENDER_WINDOW(state.currentUser), state.recurringRenderDays);
}

// Fetch a whole month of prayer times in a single API call (much more efficient)
async function fetchPrayerTimesMonthly(year, month, lat, lng, method) {
    const m = method ?? state.prayerMethod ?? 4;
    const monthKey = `month_${year}_${month}_${lat}_${lng}_m${m}`;
    if (state.prayerTimesCache[monthKey]) return; // already fetched
    try {
        const res = await fetch(`https://api.aladhan.com/v1/calendar/${year}/${month}?latitude=${lat}&longitude=${lng}&method=${m}`);
        const data = await res.json();
        if (data.code === 200 && data.data) {
            data.data.forEach(dayData => {
                const parts = dayData.date.gregorian.date.split('-'); // DD-MM-YYYY
                const dateStr = `${parts[2]}-${parts[1]}-${parts[0]}`;
                const ck = `${dateStr}_${lat}_${lng}_m${m}`;
                state.prayerTimesCache[ck] = dayData.timings;
            });
            state.prayerTimesCache[monthKey] = true;
            savePrayerTimesCache();
        }
    } catch (e) { console.warn('Monthly prayer fetch failed:', e); }
}

async function fetchPrayerTimesMonthlyByCity(year, month, city, country, method) {
    const m = method ?? state.prayerMethod ?? 4;
    const monthKey = `month_${year}_${month}_${city}_${country}_m${m}`;
    if (state.prayerTimesCache[monthKey]) return;
    try {
        const res = await fetch(`https://api.aladhan.com/v1/calendarByCity/${year}/${month}?city=${encodeURIComponent(city)}&country=${encodeURIComponent(country)}&method=${m}`);
        const data = await res.json();
        if (data.code === 200 && data.data) {
            data.data.forEach(dayData => {
                const parts = dayData.date.gregorian.date.split('-');
                const dateStr = `${parts[2]}-${parts[1]}-${parts[0]}`;
                const ck = `${dateStr}_${city}_${country}_m${m}`;
                state.prayerTimesCache[ck] = dayData.timings;
            });
            state.prayerTimesCache[monthKey] = true;
            savePrayerTimesCache();
        }
    } catch (e) { console.warn('Monthly prayer fetch by city failed:', e); }
}

// Pre-fetch prayer times for the entire render window (1 call per month needed)
async function prefetchPrayerTimesForWindow(startDateStr, endDateStr) {
    ...
    for (let d = new Date(start); d <= end; d.setMonth(d.getMonth() + 1)) {
        months.add(`${d.getFullYear()}_${d.getMonth() + 1}`);
    }
    const promises = [];
    months.forEach(key => {
        const [y, mo] = key.split('_').map(Number);
        if (loc.city && loc.country) {
            promises.push(fetchPrayerTimesMonthlyByCity(y, mo, loc.city, loc.country, m));
        } else if (loc.lat && loc.lng) {
            promises.push(fetchPrayerTimesMonthly(y, mo, loc.lat, loc.lng, m));
        }
    });
    if (promises.length > 0) {
        await Promise.all(promises);
        renderCurrentView(); // Re-render once prayer times are cached
    }
}

// Resolve prayer-based event times from cache for a specific date (sync, no fetch)
function resolvePrayerEventTimes(event, dateStr) {
    if (!event.isPrayerBased || !event.prayerConfig) return event;
    const loc = state.userLocation;
    if (!loc) return event;
    const m = state.prayerMethod ?? 4;
    const cacheKey = (loc.city && loc.country)
        ? `${dateStr}_${loc.city}_${loc.country}_m${m}`
        : `${dateStr}_${loc.lat}_${loc.lng}_m${m}`;
    const timings = state.prayerTimesCache[cacheKey];
    if (!timings) return event; // not yet cached — use stored fallback times
    const { prayer, startOffset, endOffset } = event.prayerConfig;
    const prayerTimeStr = timings[prayer];
    if (!prayerTimeStr) return event;
    const prayerMins = getPrayerTimeInMinutes(prayerTimeStr);
    return {
        ...event,
        startTime: minutesToTimeString(prayerMins + (startOffset || 0)),
        endTime:   minutesToTimeString(prayerMins + (endOffset  || 30))
    };
}

// Generate occurrences for a single base event within [startDateStr, endDateStr]
// Returns virtual (in-memory only) event instances
function generateInstancesInRange(baseEvent, startDateStr, endDateStr) {
    const instances = [];
    ...
    if (baseDate > rangeEnd) return instances;
    ...
    while (idx < SAFETY) {
        ...
        switch (baseEvent.repeat) {
            ...
            case 'weekly':
                ...
                break;
            case 'monthly':
                nextDate = addMonthsSafe(baseDate, idx, snap);
                if (!nextDate) continue;
                break;
            case 'yearly': {
                const ty = baseDate.getFullYear() + idx;
            ...
            case 'weekdays': {
                ...
                while (wAdded < idx) {
                    ...
                    if (nextDate.getDay() !== 0 && nextDate.getDay() !== 6) wAdded++;
                }
                break;
            }
            case 'custom': {
                ...
                if (unit === 'weeks' && specificDays) {
                    ...
                    while (found < idx) {
                        ...
                        const wDiff = Math.floor((sd - baseDate) / (7 * 86400000));
                        if (wDiff % interval === 0 && specificDays.includes(sd.getDay())) {
                            found++;
                            nextDate = new Date(sd);
                        }
                    }
                } else {
                    switch (unit) {
                        case 'days':
                            nextDate = new Date(baseDate);
                            nextDate.setDate(baseDate.getDate() + interval * idx);
                            break;
                        case 'weeks':
                            nextDate = new Date(baseDate);
                            nextDate.setDate(baseDate.getDate() + interval * 7 * idx);
                            break;
                        case 'months':
                            nextDate = addMonthsSafe(baseDate, interval * idx, snap);
                            if (!nextDate) continue;
                            break;
                        case 'years': {
                            const ty = baseDate.getFullYear() + interval * idx;
                            const dim = new Date(ty, baseDate.getMonth() + 1, 0).getDate();
                            nextDate = new Date(ty, baseDate.getMonth(), Math.min(baseDate.getDate(), dim), 12, 0, 0);
                            break;
                        }
        ...
        if (!nextDate) break;
        if (endType === 'after' && idx > endCount - 1) break;
        if (endType === 'on' && endDate && nextDate > endDate) break;
        if (nextDate > rangeEnd) break;
        if (nextDate < rangeStart) continue; // skip, but keep going

        const instance = { ...baseEvent };
        // Deterministic ID: won't clash with stored events
        instance.id = baseEvent.id + '_' + dateToLocalString(nextDate);
        instance.date = dateToLocalString(nextDate);
        instance.parentId = baseEvent.id;
        instance.completed = false;
        instance._virtual = true;
        instances.push(instance);
    }
    return instances;
}

// Expand all lazy recurring events for the given date range
function expandRecurringForRange(startDateStr, endDateStr) {
    const lazyBases = events.filter(e =>
        e.lazyRepeat && e.repeat && e.repeat !== 'none' && !e.parentId && !e._virtual
    );
    const result = [];
    for (const base of lazyBases) {
        result.push(...generateInstancesInRange(base, startDateStr, endDateStr));
    }
    return result;
}

// Get events for a date: stored + virtual lazy expansions, with prayer times resolved from cache
function getEventsForDate(dateStr, virtualEvents) {
    const stored  = events.filter(e => e.date === dateStr && state.activeCalendars.includes(e.calendar));
    const virtual = (virtualEvents || []).filter(e => e.date === dateStr && state.activeCalendars.includes(e.calendar));
    return [...stored, ...virtual].map(e => resolvePrayerEventTimes(e, dateStr));
}

// Compute render window based on currentDate and recurringRenderDays setting
function getRenderWindowDates() {
    const start = new Date(state.currentDate);
    start.setDate(start.getDate() - 7); // 1 week back-buffer for current view
    const end = new Date(state.currentDate);
    end.setDate(end.getDate() + (state.recurringRenderDays || 30));
    return { start: dateToLocalString(start), end: dateToLocalString(end) };
}

function loadAllData() {
    loadEvents(); loadTheme(); loadBackground(); loadCalendars();
    loadLocation(); loadPrayerTimesCache(); loadJournal(); loadVariables(); loadShowPrayerTimes(); loadRenderWindow();
}
...
function saveAllData() {
    ...
    saveLocation(); savePrayerTimesCache(); saveJournal(); saveVariables(); saveShowPrayerTimes(); saveRenderWindow();
}
...
async function handleFormSubmit(e) {
    ...
    if (state.editingEventId) {
        ...
        // Mark repeating events as lazy — instances expand on-the-fly within the render window
        // This avoids storing hundreds of pre-generated instances and allows prayer times to flex daily
        if (eventData.repeat && eventData.repeat !== 'none') {
            eventData.lazyRepeat = true;
        }
    }
...
function renderMobileWeekHeader(startDay) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        headerCell.innerHTML = `
            ...
            <div class="text-xl font-semibold ${isToday ? 'text-blue-600 dark:text-blue-400' : 'theme-text'} leading-tight">${date.getDate()}</div>
            <div class="text-[10px] theme-text-secondary">${hijri.day} ${hijri.month.substring(0,3)}</div>
...
function renderMobileWeekBody(startDay) {
    ...
    // Compute virtual events once for the 3-day window
    const { start: winStart, end: winEnd } = getRenderWindowDates();
    const virtualEvents = expandRecurringForRange(winStart, winEnd);
    if (state.userLocation) prefetchPrayerTimesForWindow(winStart, winEnd);
    ...
    for (let i = 0; i < 3; i++) {
        ...
        const isToday = dateToLocalString(date) === todayLocalString();
        const dateStr = dateToLocalString(date);
        weekBody.appendChild(createDayColumn(dateStr, isToday, i, virtualEvents));
    }
}
...
function renderWeekBody(startOfWeek) {
    ...
    // Compute render window + virtual lazy events once for the whole week
    const { start: winStart, end: winEnd } = getRenderWindowDates();
    const virtualEvents = expandRecurringForRange(winStart, winEnd);
    // Trigger background prayer-time prefetch (re-renders once cache is warm)
    if (state.userLocation) prefetchPrayerTimesForWindow(winStart, winEnd);
...
function createDayColumn(dateStr, isToday, dayIndex, virtualEvents) {
    ...
    const dayEvents = getEventsForDate(dateStr, virtualEvents);
...
function renderMonthView() {
    ...
    // Compute virtual events for the whole month at once
    const monthStart = `${year}-${String(month + 1).padStart(2, '0')}-01`;
    const monthEnd   = `${year}-${String(month + 1).padStart(2, '0')}-${String(daysInMonth).padStart(2, '0')}`;
    const virtualEvents = expandRecurringForRange(monthStart, monthEnd);
    if (state.userLocation) prefetchPrayerTimesForWindow(monthStart, monthEnd);
...
function renderYearView() {
    ...
    // Compute virtual events for the whole year at once (respects render window cap)
    const yearStart = `${year}-01-01`;
    const yearEnd   = `${year}-12-31`;
    const { start: winStart, end: winEnd } = getRenderWindowDates();
    // Clip to whichever is smaller: the full year or the render window
    const effectiveEnd = winEnd < yearEnd ? winEnd : yearEnd;
    const virtualEvents = expandRecurringForRange(yearStart, effectiveEnd);
    if (state.userLocation) prefetchPrayerTimesForWindow(yearStart, effectiveEnd);
    ...
    monthNames.forEach((monthName, idx) => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const hasEvents = getEventsForDate(dateStr, virtualEvents).length > 0;
            const isToday = dateStr === todayLocalString(); // FIX: timezone-safe
    ...
    sidebar.innerHTML = `
        ...
        <div class="p-4 space-y-3 shrink-0">
            <div class="flex space-x-2">
                <button onclick="showJournalEditor()" class="flex-1 py-2 px-4 rounded-lg border theme-border hover:theme-bg-tertiary transition-colors flex items-center justify-center space-x-2 ${state.sidebarView === 'journal' ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500' : ''}">
                    <span>📝</span><span class="text-sm font-medium">Journal</span>
                </button>
                <button onclick="showScoreEditor()" class="flex-1 py-2 px-4 rounded-lg border theme-border hover:theme-bg-tertiary transition-colors flex items-center justify-center space-x-2 ${state.sidebarView === 'score' ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500' : ''}">
...
function calcAllTimeTotal() {
    return Object.values(state.journalEntries).reduce((sum, e) => sum + (e?.score || 0), 0);
}
    ...
    panel.innerHTML = `
        <div class="p-3 border-b theme-border">
            ...
            <div class="flex gap-2 mt-2 pt-2 border-t theme-border">
                <button onclick="createNewUser(); closeAccountSettings();" class="flex-1 flex items-center justify-center space-x-2 px-3 py-2 rounded-lg hover:theme-bg-tertiary transition-colors text-left text-sm text-blue-600 dark:text-blue-400">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"></path></svg>
                    <span>Add</span>
                </button>
                <button onclick="deleteUser(); closeAccountSettings();" class="flex-1 flex items-center justify-center space-x-2 px-3 py-2 rounded-lg hover:theme-bg-tertiary transition-colors text-sm text-red-600">
        ...
        <div class="p-2 space-y-1">
            ...
            <div class="px-3 py-2">
                <div class="flex items-center justify-between mb-1">
                    <div class="flex items-center space-x-2">
                        <svg class="w-4 h-4 theme-text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                        <span class="text-sm">Recurring render window</span>
                    </div>
                </div>
                <select id="renderWindowSelect" onchange="setRenderWindow(this.value)"
                    class="w-full theme-bg-tertiary border theme-border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 theme-text">
                    <option value="7"   ${state.recurringRenderDays === 7   ? 'selected' : ''}>1 week</option>
                    <option value="14"  ${state.recurringRenderDays === 14  ? 'selected' : ''}>2 weeks</option>
                    <option value="30"  ${state.recurringRenderDays === 30  ? 'selected' : ''}>1 month (default)</option>
                    <option value="90"  ${state.recurringRenderDays === 90  ? 'selected' : ''}>3 months</option>
                    <option value="180" ${state.recurringRenderDays === 180 ? 'selected' : ''}>6 months</option>
                    <option value="365" ${state.recurringRenderDays === 365 ? 'selected' : ''}>1 year</option>
                </select>
                <p class="text-[10px] theme-text-secondary mt-1 leading-tight">From current view forward. Larger = more events shown but slower.</p>
            </div>
            <div class="border-t theme-border my-1"></div>
...
function setRenderWindow(days) {
    state.recurringRenderDays = parseInt(days) || 30;
    saveRenderWindow();
    renderCurrentView();
}
